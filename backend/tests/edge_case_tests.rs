...
async fn test_price_at_minimum() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "MINPRICE", "Min Price User", dollars(100_000), vec![],
    ).await;
...
async fn test_price_at_maximum() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "MAXPRICE", "Max Price User", i64::MAX / 2, vec![],
    ).await;
...
async fn test_price_exact_match() {
    ...
    place_limit_sell(&state, seller, &symbol, 10, exact_price).await.unwrap();
    place_limit_buy(&state, buyer, &symbol, 10, exact_price).await.unwrap();
...
async fn test_quantity_of_one() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "ONEQTY", "One Qty User", dollars(100_000), vec![],
    ).await;
...
async fn test_partial_fill_remainder() {
    ...
    assert_eq!(open_buy.unwrap().remaining_qty, 50, "Should have 50 shares remaining");
}
...
async fn test_multiple_partial_fills() {
    ...
    let seller2 = create_test_user_with_portfolio(
        &state, "S2", "Seller 2", dollars(10_000),
        vec![("AAPL".to_string(), 30)],
    ).await;
    let seller3 = create_test_user_with_portfolio(
        &state, "S3", "Seller 3", dollars(10_000),
    ...
    place_limit_sell(&state, seller2, &symbol, 30, dollars(100)).await.unwrap();
    place_limit_sell(&state, seller3, &symbol, 40, dollars(100)).await.unwrap();
    ...
    assert_eq!(position.unwrap().qty, 100, "Should have received 100 shares");
}
...
async fn test_sell_exact_portfolio_qty() {
    ...
    let seller = create_test_user_with_portfolio(
        &state, "EXACT", "Exact Sell", dollars(10_000),
...
async fn test_short_position_creation() {
    ...
    // User with no shares
    let seller = create_test_user_with_portfolio(
        &state, "SHORT", "Short Seller", dollars(100_000), vec![],
    ).await;
    let buyer = create_test_user_with_portfolio(
        &state, "BUYER", "Buyer", dollars(100_000), vec![],
    ).await;
    ...
    if sell_result.is_ok() {
        ...
        if let Some(pos) = position {
            assert!(pos.short_qty > 0 || pos.qty == 0, "Should have short position or no position");
        }
    }
}
...
async fn test_empty_portfolio_sync() {
    ...
    assert!(user_data.portfolio.is_empty(), "New user should have empty portfolio");
}
...
async fn test_single_order_in_book() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "SINGLE", "Single Order User", dollars(100_000), vec![],
    ).await;
...
async fn test_max_depth_levels() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "DEEP", "Deep Book User", dollars(1_000_000), vec![],
    ).await;
    ...
    for i in 0..20u64 {
        place_limit_buy(&state, user, &symbol, 10, dollars(100) - (i as i64)).await.unwrap();
    }
...
async fn test_self_trade_allowed() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "SELF", "Self Trade User", dollars(100_000),
...
async fn test_price_time_priority() {
    ...
    assert!(buyer1_pos.is_some() || buyer2_orders.is_empty(),
...
async fn test_bid_ask_spread() {
    ...
    let buyer = create_test_user_with_portfolio(
        &state, "BUY", "Buyer", dollars(100_000), vec![],
    ).await;
    let seller = create_test_user_with_portfolio(
        &state, "SELL", "Seller", dollars(10_000),
    ...
    place_limit_buy(&state, buyer, &symbol, 10, dollars(99)).await.unwrap();
    ...
    assert!(best_ask > best_bid, "Ask should be higher than bid (no trade should occur)");
}
...
async fn test_timestamps_reasonable() {
    ...
    assert!(user.created_at <= now, "Timestamp should not be in the future");
    assert!(user.created_at > now - 60, "Timestamp should be within last minute");
}
...
async fn test_timestamp_no_overflow() {
    ...
    assert!(i64::MAX > very_far * 100, "i64 should handle timestamps far into the future");
}
...
async fn test_trade_timestamps_ordered() {
    ...
    if trades.len() >= 2 {
        ...
        assert!(monotonic_inc || monotonic_dec, "Timestamps should be monotonically ordered");
    }
}
...
async fn test_money_locked_for_orders() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "LOCK", "Lock Test User", initial_money, vec![],
    ).await;
    ...
    place_limit_buy(&state, user, &symbol, 100, dollars(100)).await.unwrap();

    let user_data = state.user_repo.find_by_id(user).await.unwrap().unwrap();

    // Money should be locked
    assert!(user_data.locked_money > 0, "Should have locked money");
    assert_eq!(user_data.locked_money, order_value, "Locked amount should match order value");

    // Available money should be reduced
    let available = user_data.money - user_data.locked_money;
    assert!(available < initial_money, "Available money should be less than initial");
}
...
async fn test_cannot_exceed_available_funds() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "FUND", "Fund Test User", dollars(100), vec![],
    ).await;
    ...
    assert!(result.is_err(), "Should reject order exceeding available funds");
}
...
async fn test_cancel_returns_locked_funds() {
    ...
    let initial_money = dollars(100_000);
    let user = create_test_user_with_portfolio(
        &state, "CANCEL", "Cancel Test User", initial_money, vec![],
    ).await;

    open_market(&state);

    // Place order
    let order_id = place_limit_buy(&state, user, &symbol, 100, dollars(100)).await.unwrap();
    ...
    assert_eq!(user_after.locked_money, 0, "Locked money should be returned");
    assert_eq!(user_after.money, initial_money, "All money should be available again");
}
...
async fn test_market_order_no_liquidity() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "NOLIQ", "No Liquidity User", dollars(100_000), vec![],
    ).await;
...
async fn test_crossing_spread() {
    ...
    place_limit_buy(&state, buyer, &symbol, 10, dollars(105)).await.unwrap();

    // Trade should execute at the ask price ($100)
    let trades = state.trade_history.get_recent_symbol_trades(&symbol, 10);
    assert!(!trades.is_empty(), "Trade should execute when crossing spread");

    // Price should be at the resting order price (the ask)
    assert_eq!(trades[0].price, dollars(100), "Trade should execute at ask price");
}
