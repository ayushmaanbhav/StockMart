...
async fn test_portfolio_state_after_buy() {
    ...
    let seller = create_test_user_with_portfolio(
        &state, "BUYSTATEST", "Buy State Seller", dollars(10_000),
        vec![("BUYSTATE".to_string(), 100)],
    ).await;
    let buyer = create_test_user_with_portfolio(
        &state, "BUYSTATEBUY", "Buy State Buyer", dollars(10_000), vec![],
    ).await;
    ...
    assert_eq!(buyer_final.money, buyer_initial.money - dollars(1_000),
        "Buyer money should decrease by trade value");
    assert_eq!(buyer_final.locked_money, 0, "No locked money after filled order");
    ...
    assert_eq!(buyer_position.unwrap().qty, 10, "Buyer should have 10 shares");
    ...
    assert_eq!(seller_final.money, seller_initial.money + dollars(1_000),
        "Seller money should increase by trade value");

    let seller_position = seller_final.portfolio.iter().find(|p| p.symbol == symbol);
    assert!(seller_position.is_some(), "Seller should still have position");
    assert_eq!(seller_position.unwrap().qty, 90, "Seller should have 90 shares left");
    assert_eq!(seller_position.unwrap().locked_qty, 0, "No locked shares after fill");
}
...
async fn test_portfolio_state_after_partial_fill() {
    ...
    let seller = create_test_user_with_portfolio(
        &state, "PARTSTATEST", "Partial State Seller", dollars(10_000),
        vec![("PARTSTATE".to_string(), 100)],
    ).await;
    let buyer = create_test_user_with_portfolio(
        &state, "PARTSTATEBUY", "Partial State Buyer", dollars(10_000), vec![],
    ).await;

    open_market(&state);

    // Seller offers 100 shares at $100
    place_limit_sell(&state, seller, &symbol, 100, dollars(100)).await.unwrap();
    ...
    let seller_position = seller_user.portfolio.iter().find(|p| p.symbol == symbol).unwrap();
    ...
    assert_eq!(seller_position.qty, 70, "Seller has 70 shares after selling 30");
    assert_eq!(seller_position.locked_qty, 70, "70 shares still locked for order");
    ...
    let buyer_position = buyer_user.portfolio.iter().find(|p| p.symbol == symbol).unwrap();
...
async fn test_portfolio_state_after_cancel() {
    ...
    let buyer = create_test_user_with_portfolio(
        &state, "CANCELSTBUY", "Cancel State Buyer", dollars(10_000), vec![],
    ).await;
    ...
    let buyer_final = state.user_repo.find_by_id(buyer).await.unwrap().unwrap();
    assert_eq!(buyer_final.locked_money, 0, "Locked money should be released");
    assert_eq!(buyer_final.money, buyer_initial.money, "Money should be restored");
}
...
async fn test_multiple_position_portfolio_state() {
    ...
    let seller = create_test_user_with_portfolio(
        &state, "MULTISELL", "Multi Seller", dollars(10_000),
        vec![
    ...
    let buyer = create_test_user_with_portfolio(
        &state, "MULTIBUY", "Multi Buyer", dollars(100_000), vec![],
    ).await;
    ...
    place_limit_sell(&state, seller, &symbol2, 20, dollars(50)).await.unwrap();
    place_limit_buy(&state, buyer, &symbol2, 20, dollars(50)).await.unwrap();

    place_limit_sell(&state, seller, &symbol3, 30, dollars(25)).await.unwrap();
    place_limit_buy(&state, buyer, &symbol3, 30, dollars(25)).await.unwrap();

    // Verify buyer has all three positions
    let buyer_user = state.user_repo.find_by_id(buyer).await.unwrap().unwrap();
    assert_eq!(buyer_user.portfolio.len(), 3, "Buyer should have 3 positions");

    // Verify correct quantities
    assert_eq!(buyer_user.portfolio.iter().find(|p| p.symbol == symbol1).unwrap().qty, 10);
    assert_eq!(buyer_user.portfolio.iter().find(|p| p.symbol == symbol2).unwrap().qty, 20);
    assert_eq!(buyer_user.portfolio.iter().find(|p| p.symbol == symbol3).unwrap().qty, 30);

    // Verify seller positions reduced
    let seller_user = state.user_repo.find_by_id(seller).await.unwrap().unwrap();
    assert_eq!(seller_user.portfolio.iter().find(|p| p.symbol == symbol1).unwrap().qty, 90);
    assert_eq!(seller_user.portfolio.iter().find(|p| p.symbol == symbol2).unwrap().qty, 180);
    assert_eq!(seller_user.portfolio.iter().find(|p| p.symbol == symbol3).unwrap().qty, 270);
}
...
async fn test_orderbook_state_multiple_orders() {
    ...
    let buyer1 = create_test_user_with_portfolio(&state, "BOOKST1", "Book 1", dollars(100_000), vec![]).await;
    let buyer2 = create_test_user_with_portfolio(&state, "BOOKST2", "Book 2", dollars(100_000), vec![]).await;
    let buyer3 = create_test_user_with_portfolio(&state, "BOOKST3", "Book 3", dollars(100_000), vec![]).await;
    ...
    place_limit_buy(&state, buyer3, &symbol, 30, dollars(98)).await.unwrap();
...
async fn test_orderbook_state_after_trade() {
    ...
    let seller = create_test_user_with_portfolio(
        &state, "BOOKTRSELL", "Book Trade Seller", dollars(10_000),
        vec![("BOOKTR".to_string(), 100)],
    ).await;
    let buyer = create_test_user_with_portfolio(
        &state, "BOOKTRBUY", "Book Trade Buyer", dollars(100_000), vec![],
    ).await;
...
async fn test_orderbook_state_after_cancel() {
    ...
    let buyer = create_test_user_with_portfolio(
        &state, "BOOKCANCBUY", "Book Cancel Buyer", dollars(100_000), vec![],
    ).await;
...
async fn test_orderbook_ask_state() {
    ...
    let seller1 = create_test_user_with_portfolio(
        &state, "BOOKASK1", "Book Ask 1", dollars(10_000),
        vec![("BOOKASK".to_string(), 100)],
    ).await;
    let seller2 = create_test_user_with_portfolio(
        &state, "BOOKASK2", "Book Ask 2", dollars(10_000),
        vec![("BOOKASK".to_string(), 100)],
    ).await;
    ...
    place_limit_sell(&state, seller1, &symbol, 10, dollars(102)).await.unwrap();
    place_limit_sell(&state, seller2, &symbol, 20, dollars(101)).await.unwrap();
...
async fn test_trade_history_state_single() {
    ...
    let seller = create_test_user_with_portfolio(
        &state, "HISTSINGSELL", "History Single Seller", dollars(10_000),
        vec![("HISTSING".to_string(), 100)],
    ).await;
    let buyer = create_test_user_with_portfolio(
        &state, "HISTSINGBUY", "History Single Buyer", dollars(100_000), vec![],
    ).await;
    ...
    // Execute trade
    place_limit_sell(&state, seller, &symbol, 15, dollars(123)).await.unwrap();
    place_limit_buy(&state, buyer, &symbol, 15, dollars(123)).await.unwrap();
...
async fn test_trade_history_ordering() {
    ...
    let seller = create_test_user_with_portfolio(
        &state, "HISTORDSELL", "History Order Seller", dollars(10_000),
        vec![("HISTORD".to_string(), 100)],
    ).await;
    let buyer = create_test_user_with_portfolio(
        &state, "HISTORDBUY", "History Order Buyer", dollars(100_000), vec![],
    ).await;
    ...
    place_limit_sell(&state, seller, &symbol, 10, dollars(101)).await.unwrap();
    place_limit_buy(&state, buyer, &symbol, 10, dollars(101)).await.unwrap();

    place_limit_sell(&state, seller, &symbol, 10, dollars(102)).await.unwrap();
    place_limit_buy(&state, buyer, &symbol, 10, dollars(102)).await.unwrap();
...
async fn test_trade_history_user_indexing() {
    ...
    let seller = create_test_user_with_portfolio(
        &state, "HISTUSRSELL", "History User Seller", dollars(10_000),
        vec![("HISTUSR".to_string(), 100)],
    ).await;
    let buyer1 = create_test_user_with_portfolio(
        &state, "HISTUSRBUY1", "History User Buyer 1", dollars(100_000), vec![],
    ).await;
    let buyer2 = create_test_user_with_portfolio(
        &state, "HISTUSRBUY2", "History User Buyer 2", dollars(100_000), vec![],
    ).await;
    ...
    // Buyer2 does 1 trade
    place_limit_sell(&state, seller, &symbol, 10, dollars(100)).await.unwrap();
    place_limit_buy(&state, buyer2, &symbol, 10, dollars(100)).await.unwrap();
...
async fn test_open_orders_tracking() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "OPENORDUSER", "Open Order User", dollars(100_000), vec![],
    ).await;

    open_market(&state);

    // Place multiple orders
    let order1 = place_limit_buy(&state, user, &symbol, 10, dollars(100)).await.unwrap();
    let order2 = place_limit_buy(&state, user, &symbol, 20, dollars(99)).await.unwrap();
    let order3 = place_limit_buy(&state, user, &symbol, 30, dollars(98)).await.unwrap();
    ...
    assert!(user_orders.iter().any(|o| o.order_id == order1 && o.qty == 10));
    assert!(user_orders.iter().any(|o| o.order_id == order2 && o.qty == 20));
    assert!(user_orders.iter().any(|o| o.order_id == order3 && o.qty == 30));
}
...
async fn test_order_status_transitions() {
    ...
    let seller = create_test_user_with_portfolio(
        &state, "ORDSTATSELL", "Order Status Seller", dollars(10_000),
        vec![("ORDSTAT".to_string(), 100)],
    ).await;
    let buyer = create_test_user_with_portfolio(
        &state, "ORDSTATBUY", "Order Status Buyer", dollars(100_000), vec![],
    ).await;
    ...
    assert_eq!(order.status, stockmart_backend::domain::models::OrderStatus::Open);
    ...
    // Full fill
    place_limit_buy(&state, buyer, &symbol, 70, dollars(100)).await.unwrap();

    // After full fill, order removed from tracking
    let order = state.orders.get_order(sell_id);
    assert!(order.is_none(), "Filled order should be removed from tracking");
}
...
async fn test_money_conservation() {
    ...
    let seller = create_test_user_with_portfolio(
        &state, "MONEYSELL", "Money Seller", dollars(10_000),
        vec![("MONEYCONS".to_string(), 100)],
    ).await;
    let buyer = create_test_user_with_portfolio(
        &state, "MONEYBUY", "Money Buyer", dollars(50_000), vec![],
    ).await;
    ...
    // Execute trade
    place_limit_sell(&state, seller, &symbol, 25, dollars(150)).await.unwrap();
    place_limit_buy(&state, buyer, &symbol, 25, dollars(150)).await.unwrap();
...
async fn test_share_conservation() {
    ...
    let seller = create_test_user_with_portfolio(
        &state, "SHARESELL", "Share Seller", dollars(10_000),
        vec![("SHARECONS".to_string(), 100)],
    ).await;
    let buyer = create_test_user_with_portfolio(
        &state, "SHAREBUY", "Share Buyer", dollars(50_000), vec![],
    ).await;
    ...
    let seller_shares_before = seller_before.portfolio.iter().find(|p| p.symbol == symbol).map(|p| p.qty).unwrap_or(0);
    let buyer_shares_before = buyer_before.portfolio.iter().find(|p| p.symbol == symbol).map(|p| p.qty).unwrap_or(0);
    let total_before = seller_shares_before + buyer_shares_before;

    open_market(&state);

    // Execute trade
    place_limit_sell(&state, seller, &symbol, 40, dollars(100)).await.unwrap();
    place_limit_buy(&state, buyer, &symbol, 40, dollars(100)).await.unwrap();

    // Total shares after
    let seller_after = state.user_repo.find_by_id(seller).await.unwrap().unwrap();
    let buyer_after = state.user_repo.find_by_id(buyer).await.unwrap().unwrap();

    let seller_shares_after = seller_after.portfolio.iter().find(|p| p.symbol == symbol).map(|p| p.qty).unwrap_or(0);
    let buyer_shares_after = buyer_after.portfolio.iter().find(|p| p.symbol == symbol).map(|p| p.qty).unwrap_or(0);
    let total_after = seller_shares_after + buyer_shares_after;

    assert_eq!(total_before, total_after, "Total shares should be conserved");
}
...
async fn test_locked_money_equals_orders() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "LOCKUSER", "Lock User", dollars(100_000), vec![],
    ).await;

    open_market(&state);

    // Place multiple buy orders
    place_limit_buy(&state, user, &symbol, 10, dollars(100)).await.unwrap(); // $1,000
    place_limit_buy(&state, user, &symbol, 20, dollars(50)).await.unwrap();  // $1,000
    place_limit_buy(&state, user, &symbol, 5, dollars(200)).await.unwrap();  // $1,000
...
async fn test_locked_shares_equals_orders() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "LOCKSHAREUSER", "Lock Share User", dollars(10_000),
    ...
    place_limit_sell(&state, user, &symbol, 20, dollars(100)).await.unwrap();
    place_limit_sell(&state, user, &symbol, 30, dollars(110)).await.unwrap();

    // Total locked shares should equal total sell order quantity
    let user_data = state.user_repo.find_by_id(user).await.unwrap().unwrap();
    let position = user_data.portfolio.iter().find(|p| p.symbol == symbol).unwrap();
...
async fn test_available_balance_calculation() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "AVAILUSER", "Available User", dollars(10_000), vec![],
    ).await;

    open_market(&state);

    // Place order locking some funds
    place_limit_buy(&state, user, &symbol, 10, dollars(500)).await.unwrap(); // Lock $5,000
