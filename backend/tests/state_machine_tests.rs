...
async fn test_order_starts_open() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "SM1", "State Machine User", dollars(100_000), vec![],
    ).await;
...
async fn test_order_partial_fill_state() {
    ...
    let buy_order_id = place_limit_buy(&state, buyer, &symbol, 100, dollars(100)).await.unwrap();
    ...
    assert_eq!(order.status, OrderStatus::Partial, "Order should be Partial after partial fill");
...
async fn test_order_cancel_state() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "CANCEL", "Cancel User", dollars(100_000), vec![],
    ).await;
    ...
    // Cancel the order
    let cancelled = state.engine.cancel_order(user, &symbol, order_id).await.unwrap();

    assert_eq!(cancelled.status, OrderStatus::Cancelled, "Cancelled order should have Cancelled status");
}
...
async fn test_filled_order_immutable() {
    ...
    let sell_order_id = place_limit_sell(&state, seller, &symbol, 100, dollars(100)).await.unwrap();
    place_limit_buy(&state, buyer, &symbol, 100, dollars(100)).await.unwrap();

    // Try to cancel filled order - should fail or order not found
    let cancel_result = state.engine.cancel_order(seller, &symbol, sell_order_id).await;
...
async fn test_cancelled_order_immutable() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "IMMUT", "Immutable User", dollars(100_000), vec![],
    ).await;
...
async fn test_market_starts_open() {
    ...
    assert!(state.engine.is_market_open(), "Market should start open by default");
}
...
async fn test_market_can_be_opened() {
    ...
    assert!(state.engine.is_market_open(), "Market should be open after toggle");
}
...
async fn test_market_can_be_closed() {
    ...
    assert!(!state.engine.is_market_open(), "Market should be closed after toggle");
}
...
async fn test_orders_rejected_market_closed() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "CLOSED", "Closed Market User", dollars(100_000), vec![],
    ).await;
    ...
    assert!(result.is_err(), "Orders should be rejected when market is closed");
}
...
async fn test_orders_accepted_market_open() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "OPEN", "Open Market User", dollars(100_000), vec![],
    ).await;

    open_market(&state);

    let result = place_limit_buy(&state, user, &symbol, 10, dollars(100)).await;
    assert!(result.is_ok(), "Orders should be accepted when market is open");
}
...
async fn test_invariant_money_non_negative() {
    ...
    // Execute many trades
    for i in 0..10 {
        place_limit_sell(&state, seller, &symbol, 10, dollars(100) + i).await.unwrap();
        place_limit_buy(&state, buyer, &symbol, 10, dollars(100) + i).await.unwrap();
    }
    ...
    assert!(seller_data.money >= 0, "Seller money should be non-negative");
    assert!(buyer_data.money >= 0, "Buyer money should be non-negative");
    assert!(seller_data.locked_money >= 0, "Seller locked_money should be non-negative");
    assert!(buyer_data.locked_money >= 0, "Buyer locked_money should be non-negative");
}
...
async fn test_invariant_locked_money_bounded() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "LOCK", "Lock Test User", dollars(100_000), vec![],
    ).await;
    ...
    for i in 0..5 {
        place_limit_buy(&state, user, &symbol, 10, dollars(100) + i).await.unwrap();
    }

    let user_data = state.user_repo.find_by_id(user).await.unwrap().unwrap();

    // Locked money should not exceed total money
    assert!(user_data.locked_money <= user_data.money,
        "Locked money {} should not exceed total money {}",
...
async fn test_invariant_cannot_overspend() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "OVERSPEND", "Overspend User", dollars(1_000), vec![],
    ).await;
...
async fn test_invariant_trade_preserves_value() {
    ...
    // Execute trade
    place_limit_sell(&state, seller, &symbol, trade_qty, trade_price).await.unwrap();
    place_limit_buy(&state, buyer, &symbol, trade_qty, trade_price).await.unwrap();
    ...
    assert_eq!(total_money_before, total_money_after,
...
async fn test_invariant_bids_sorted_descending() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "SORT", "Sort Test User", dollars(1_000_000), vec![],
    ).await;
    ...
    place_limit_buy(&state, user, &symbol, 10, dollars(105)).await.unwrap();
    place_limit_buy(&state, user, &symbol, 10, dollars(95)).await.unwrap();
    place_limit_buy(&state, user, &symbol, 10, dollars(110)).await.unwrap();
...
async fn test_invariant_asks_sorted_ascending() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "SORT", "Sort Test User", dollars(10_000),
    ...
    place_limit_sell(&state, user, &symbol, 10, dollars(100)).await.unwrap();
    place_limit_sell(&state, user, &symbol, 10, dollars(95)).await.unwrap();
    place_limit_sell(&state, user, &symbol, 10, dollars(110)).await.unwrap();
    place_limit_sell(&state, user, &symbol, 10, dollars(105)).await.unwrap();
...
async fn test_invariant_no_crossed_book() {
    ...
    place_limit_buy(&state, buyer, &symbol, 5, dollars(105)).await.unwrap();  // Crosses at $100
...
async fn test_invariant_order_quantities_positive() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "QTY", "Quantity Test User", dollars(100_000), vec![],
    ).await;

    open_market(&state);

    place_limit_buy(&state, user, &symbol, 10, dollars(100)).await.unwrap();
    place_limit_buy(&state, user, &symbol, 20, dollars(99)).await.unwrap();

    let depth = state.engine.get_order_book_depth(&symbol, 10);
    let (bids, _) = depth.unwrap();

    for (price, qty) in bids {
        assert!(qty > 0, "Order quantity at price {} should be positive", price);
    }
}
...
async fn test_invariant_trade_valid_participants() {
    ...
    let maker_exists = state.user_repo.find_by_id(trade.maker_user_id).await.unwrap().is_some();
    let taker_exists = state.user_repo.find_by_id(trade.taker_user_id).await.unwrap().is_some();
...
async fn test_invariant_trade_price_valid() {
    ...
    place_limit_sell(&state, seller, &symbol, 10, sell_price).await.unwrap();
    place_limit_buy(&state, buyer, &symbol, 10, buy_price).await.unwrap();
    ...
    assert_eq!(trade.price, sell_price,
...
async fn test_invariant_trade_updates_portfolios() {
    ...
    let seller_shares_before = seller_before.portfolio.iter()
        .find(|p| p.symbol == "PORT")
        .map(|p| p.qty)
        .unwrap_or(0);
    let buyer_shares_before = buyer_before.portfolio.iter()
        .find(|p| p.symbol == "PORT")
        .map(|p| p.qty)
        .unwrap_or(0);
    ...
    let trade_qty = 50u64;
    place_limit_sell(&state, seller, &symbol, trade_qty, dollars(100)).await.unwrap();
    place_limit_buy(&state, buyer, &symbol, trade_qty, dollars(100)).await.unwrap();

    let seller_after = state.user_repo.find_by_id(seller).await.unwrap().unwrap();
    let buyer_after = state.user_repo.find_by_id(buyer).await.unwrap().unwrap();

    let seller_shares_after = seller_after.portfolio.iter()
        .find(|p| p.symbol == "PORT")
        .map(|p| p.qty)
        .unwrap_or(0);
    let buyer_shares_after = buyer_after.portfolio.iter()
        .find(|p| p.symbol == "PORT")
        .map(|p| p.qty)
        .unwrap_or(0);

    // Seller should have fewer shares
    assert_eq!(seller_shares_after, seller_shares_before - trade_qty,
        "Seller shares should decrease by trade qty");

    // Buyer should have more shares
    assert_eq!(buyer_shares_after, buyer_shares_before + trade_qty,
