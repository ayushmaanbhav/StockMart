...
async fn test_trading_without_auth() {
    ...
    assert!(result.is_err(), "Should reject order from non-existent user");
}
...
async fn test_admin_action_without_auth() {
    ...
    let user = state.user_repo.find_by_id(trader_id).await.unwrap().unwrap();
    assert!(!user.role.can_control_market(), "Trader should not have market control permission");
}
...
async fn test_token_not_predictable() {
    ...
    assert!(token1.len() >= 32, "Token should be cryptographically strong length");
    assert!(token2.len() >= 32, "Token should be cryptographically strong length");

    // Token should be hex-encoded (all chars are valid hex digits)
    assert!(token1.chars().all(|c| c.is_ascii_hexdigit()), "Token should be hex-encoded");
}
...
async fn test_token_validation() {
    ...
    assert_eq!(result.unwrap(), user_id, "Token should return correct user_id");
}
...
async fn test_revoked_token_invalid() {
    ...
    assert!(state.tokens.validate_token(&token).is_none(), "Revoked token should not validate");
}
...
async fn test_cannot_cancel_other_users_order() {
    ...
    let user1 = create_test_user_with_portfolio(
        &state, "USER1", "User 1", dollars(100_000), vec![],
    ).await;
    let user2 = create_test_user_with_portfolio(
        &state, "USER2", "User 2", dollars(100_000), vec![],
    ).await;
    ...
    assert!(cancel_result.is_err(), "Should not be able to cancel another user's order");
...
async fn test_non_admin_cannot_admin_action() {
    ...
    assert!(!trader.role.can_control_market(), "Trader cannot control market");
    assert!(!trader.role.can_manage_users(), "Trader cannot manage users");
    assert!(!trader.role.can_manage_companies(), "Trader cannot manage companies");
...
async fn test_cannot_sell_shares_not_owned() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "NOSELL", "No Sell User", dollars(100_000), vec![],
    ).await;
    ...
    if result.is_ok() {
        ...
        assert!(pos.short_qty > 0 || pos.qty == 0, "Should be short if sell allowed without shares");
    }
...
async fn test_user_trade_history_isolation() {
    ...
    assert!(!seller_trades.trades.is_empty() || !buyer_trades.trades.is_empty(),
...
async fn test_very_large_quantity() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "BIGQTY", "Big Quantity User", i64::MAX / 2, vec![],
    ).await;
...
async fn test_negative_price_handling() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "NEGPRICE", "Neg Price User", dollars(100_000), vec![],
    ).await;
...
async fn test_empty_strings_handled() {
    ...
    // create_company takes (symbol, name, sector, volatility)
    let result2 = state.admin.create_company(
        "".to_string(),
...
async fn test_zero_quantity_handling() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "ZEROQTY", "Zero Qty User", dollars(100_000), vec![],
    ).await;
...
async fn test_zero_price_handling() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "ZEROPRICE", "Zero Price User", dollars(100_000), vec![],
    ).await;
...
async fn test_rapid_order_placement() {
    ...
    let user = create_test_user_with_portfolio(
        &state, "RAPIDORDER", "Rapid Order User", dollars(1_000_000), vec![],
    ).await;
...
async fn test_chat_history_bounded() {
    ...
    let history = state.chat.get_history();
    assert!(history.len() <= 50, "Chat history should be bounded to 50 messages");
}
...
async fn test_password_not_exposed() {
    ...
    for entry in leaderboard {
        ...
        assert!(!entry.name.contains("secret"), "Name should not contain password");
    }
}
...
async fn test_portfolio_isolation() {
    ...
    let _user1 = create_test_user_with_portfolio(
        &state, "PORT1", "Portfolio User 1", dollars(100_000),
        vec![("AAPL".to_string(), 100)],
    ).await;
    let user2 = create_test_user_with_portfolio(
        &state, "PORT2", "Portfolio User 2", dollars(50_000),
    ...
    assert!(user2_data.portfolio.iter().all(|p| p.symbol == "GOOGL"),
        "User2 should only see their own positions");
    assert!(!user2_data.portfolio.iter().any(|p| p.symbol == "AAPL"),
...
async fn test_token_limit_per_user() {
    ...
    assert!(state.tokens.validate_token(&token1).is_some(), "First token should be valid");
    ...
    assert_eq!(revoked2[0], token1, "Revoked token should be the first token");

    // First token should no longer be valid
    assert!(state.tokens.validate_token(&token1).is_none(), "First token should be revoked");

    // Second token should be valid
    assert!(state.tokens.validate_token(&token2).is_some(), "Second token should be valid");
}
